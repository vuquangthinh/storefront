            ...
            <div className="sidebar-content pb-0 pb-lg-4">
                <div>
                    <div className="filter-actions">
                        ...
                        <ALink href={ { pathname, query: { type: query.type ? query.type : null } } } className="filter-clean" scroll={ false } >Clean All</ALink>
                    </div>
                            ...
                            <ul className="widget-body filter-items">
                                {
                                    filterData.sizes.map( ( item, index ) =>
                                        <li
                                            ...
                                            key={ item + ' - ' + index }
                                        >
                                            <ALink href={ { pathname, query: { ...query, page: 1, sizes: getUrlForAttrs( 'sizes', item.slug ), type: query.type ? query.type : null } } } scroll={ false }>{ item.name }
                                            </ALink>
                                        </li>
                                    )
                                }
                            ...
                            <ul className="widget-body filter-items">
                                {
                                    filterData.colors.map( ( item, index ) =>
                                        <li
                                            ...
                                            key={ item + ' - ' + index }
                                        >
                                            <ALink href={ { pathname, query: { ...query, page: 1, colors: getUrlForAttrs( 'colors', item.slug ), type: query.type ? query.type : null } } } scroll={ false }>{ item.name }
                                            </ALink>
                                        </li>
                                    )
                                }
                            ...
                            <ul className="widget-body filter-items filter-price">
                                {
                                    prices.map( ( price, index ) =>
                                        ...
                                        >
                                            <ALink href={ { pathname, query: { ...query, page: 1, min_price: getUrlForAttrs( 'min_price', price.min ), max_price: getUrlForAttrs( 'max_price', price.max ), type: query.type ? query.type : null } } } scroll={ false }>
                                                {
                            ...
                            <div className="widget-body pt-2">
                                {
                                    filterData.tag.map( ( item, index ) =>
                                        <ALink scroll={ false } href={ { pathname, query: { ...query, page: 1, tag: getUrlForAttrs( 'tag', item.slug ), type: query.type ? query.type : null } } } className={ `${ containsAttrInUrl( 'tag', item.slug ) ? 'active' : '' } tag` } key={ "tag-" + index }>{ item.name }
                                        </ALink>
                                    )
                                }
                            </div>
