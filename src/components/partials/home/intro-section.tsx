                    ...
                    <div className="container">
                        <div className={`banner-content ${s.align === 'right' ? 'ml-auto mr-0' : 'ml-lg-2'} y-50`}>
                            <Reveal keyframes={ fadeInRightShorter } delay={ 300 } duration={ 800 }>
                                <h4 className={`banner-subtitle text-uppercase${idx === 1 ? ' text-primary' : ''}`}>{s.subtitle}</h4>
                                <h3 className="banner-title ls-m text-dark font-weight-bolder" dangerouslySetInnerHTML={{ __html: s.titleHtml }}></h3>
                                {s.buttonText && s.buttonHref ? (
                                    <ALink href={s.buttonHref} className="btn btn-dark btn-outline btn-rounded mb-1">{s.buttonText} <i className="d-icon-arrow-right"></i></ALink>
                                ) : null}
                            </Reveal>
                        </div>
                    </div>
