...
const OwlCarousel = forwardRef<OwlCarouselHandle, OwlCarouselProps>(
  ({ adClass = "", options = {}, events, onChangeRef, onChangeIndex, children }, ref) => {
    ...
    return (
      <div ref={containerRef} className={`${adClass}`}>
        <Swiper
          modules={[
        ...
        {dotsEnabled && pageCount > 1 && <div ref={dotsRef} className="owl-dots" />}
      </div>
    );
  }
);
