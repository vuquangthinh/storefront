            ...
            {totalPage > 1 &&
                <ul className="pagination">
                    <li className={`page-item ${page < 2 ? 'disabled' : ''}`}>
                        <ALink className="page-link page-link-prev" href={page > 1 ? { pathname: pathname, query: { ...query, page: page - 1 } } : '#'} scroll={false}>
                            ...
                            (index === 1 && item > 2) ?
                                <React.Fragment key={`page-${index}`}>
                                    ...
                                    <li className={`page-item ${page === item ? 'active' : ''}`} >
                                        <ALink className="page-link" href={page === item ? '#' : { pathname: pathname, query: { ...query, page: item } }} scroll={false}>
                    ...
                    <li className={`page-item ${page > totalPage - 1 ? 'disabled' : ''}`}>
                        <ALink className="page-link page-link-next" href={page < totalPage ? { pathname: pathname, query: { ...query, page: page + 1 } } : '#'} scroll={false}>
